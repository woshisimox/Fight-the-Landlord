...
export default async function handler(req: NextApiRequest, res: NextApiResponse){
  try{
    ...
    if (provider==='openai' && (keys?.openaiKey)){ return res.status(200).json(await callOpenAI(keys.openaiKey, snapshot)); }
    if (provider==='kimi'   && (keys?.kimiKey))  { return res.status(200).json(await callKimi(keys.kimiKey, snapshot)); }
    if (provider==='grok'   && (keys?.grokKey))  { return res.status(200).json(await callGrok(keys.grokKey, snapshot)); }
