...
for await (const ev of iter as any) {
  ...
  try {
    ...
    const perSeat = [0,1,2].map(i => {
      const s = arr[i] || { plays:0, passes:0, bombs:0, rockets:0, cards:0 };
...
writeLine(res, { type:'log', message:`开始连打 ${rounds} 局（four2=${four2}）…` });
  ...
  try {
    ...
    writeLine(res, { type:'event', kind:'stats', perSeat });
      ...
      const roundBots = baseBots.map((bot, i) => traceWrap(
        seatSpecs[i]?.choice as BotChoice,
        seatSpecs[i],
        bot,
        res,
        (_seat:number, text?:string)=> onReason(i, text),
        (_seat:number, mv:any)=> onMove(i, mv),
        turnTimeoutMsArr[i]
      ));
