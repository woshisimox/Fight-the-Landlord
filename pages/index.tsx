...
function LivePanel(props: LiveProps) {
  ...
  const roundsRef = useRef(props.rounds);
  useEffect(() => {
    roundsRef.current = props.rounds;
  }, [props.rounds]);

  // 标记本局是否已计数，避免重复累加 finished
  const roundCountedRef = useRef(false);
  const lastReasonRef = useRef<(string | null)[]>([null, null, null]);

  // —— 把对象 pretty JSON 分多行写入日志
  const dumpJSONLines = (obj: any, label?: string, maxLines = 200) => {
    try {
      ...
      return label ? [`—— ${label}（无法序列化） ——`] : ['（无法序列化）'];
    }
  };
  ...
  const start = async () => {
    ...
    const buildSeatSpecs = (): any[] =>
      props.seats.slice(0, 3).map((choice, i) => {
    ...
    const playOneGame = async (labelRoundNo: number) => {
      controllerRef.current?.abort(); // 保险：清理遗留连接
      controllerRef.current = new AbortController();
      roundCountedRef.current = false;

      setLog((l) => [
        ...l,
        `【前端】开始第 ${labelRoundNo} 局 | 座位: ${seatSummaryText(buildSeatSpecs())} | coop=${
          props.farmerCoop ? 'on' : 'off'
        ...
        for (const raw of batch) {
          ...
          try {
            ...
            if (m.type === 'event' && m.kind === 'round-end') {
              ...
              // 兜底：如未在 win 计数，则在 round-end 记一次
            ...
            if (m.type === 'event' && m.kind === 'bot-call') {
              nextLog = [
                ...nextLog,
                `AI调用｜${seatName(m.seat)}｜${m.by}${m.model ? `(${m.model})` : ''}｜阶段=${m.phase || 'unknown'}${
                  m.need ? `｜需求=${m.need}` : ''
                }`,
              ];
            ...
            if (m.type === 'event' && m.kind === 'win') {
              ...
              nextLog = [
                ...nextLog,
                `胜者：${seatName(m.winner)}，倍数 x${m.multiplier}，当局积分（按座位） ${rot.join(' / ')}｜原始（相对地主） ${ds.join(
                  ' / '
                )}｜地主=${seatName(L)}`,
              ];
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button onClick={stop} style={{ padding: '8px 12px', borderRadius: 8 }}>
          停止
        </button>
      </div>
