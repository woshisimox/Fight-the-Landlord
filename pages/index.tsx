...
function LadderPanel() {
  ...
  useEffect(() => {
    ...
    return () => { 
      if (typeof window !== 'undefined') window.removeEventListener('ddz-all-refresh', onAny as any); 
      clearInterval(t); 
    };
  }, []);
          ...
          return (
            <div key={it.id} style={{ display: 'contents' }}>
              ...
              <div style={{ position: 'relative', height: 16, background: '#f9fafb', border: '1px solid #f3f4f6', borderRadius: 8 }}>
                <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: pct * 100 + '%', background: pos ? '#10b981' : '#ef4444', opacity: 0.3 }} />
                <div style={{ ...axisStyle, left: '50%' }} />
                <div style={{ position: 'absolute', right: pos ? 4 : '50%', top: 2, bottom: 2, width: 2, background: pos ? '#10b981' : '#ef4444' }} />
              </div>
              <div style={{ textAlign: 'right', fontSize: 12, color: it.n > 0 ? '#059669' : '#6b7280' }}>{it.val.toFixed(1)}</div>
            </div>
          );
        })}
      </div>
...
// 假设的 helper functions (从上下文推断，需要根据完整代码补充)
...
function choiceLabel(choice: BotChoice): string {
  // 示例实现
  const labels = {
    'built-in:greedy-max': 'Greedy Max',
    'built-in:greedy-min': 'Greedy Min',
    'built-in:random-legal': 'Random Legal',
    'built-in:mininet': 'MiniNet',
    'built-in:ally-support': 'Ally Support',
    'built-in:endgame-rush': 'Endgame Rush',
    'ai:openai': 'OpenAI',
    'ai:gemini': 'Gemini',
    'ai:grok': 'Grok',
    'ai:kimi': 'Kimi',
...
// LivePanel 组件 (从上下文推断，简化版)
function LivePanel(props: LiveProps) {
  // 实现 LivePanel 逻辑，这里简化
  return (
    <div>
      {/* Live panel content */}
      <p>Live Game Panel</p>
...
function Home() {
  ...
  const [startScore, setStartScore] = useState(3000);
  ...
  // 示例 handlers
  const handleAllFileUploadHome = (e: any) => {
    // 实现上传逻辑
  };

  const setSeatDelay = (i: number, val: string) => {
    const v = Number(val);
    setSeatDelayMs(arr => { const n = [...arr]; n[i] = v; return n; });
  };

  useEffect(() => {
    autoTranslateContainer(document.body, lang);
  }, [lang]);
    ...
    <LangContext.Provider value={lang}>
      <div style={{ padding: 20, maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20, marginTop: 20 }}>
          <div>
            <div style={{ fontWeight: 700, marginBottom: 10 }}>{I18N[lang].Settings}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 10, alignItems: 'center' }}>
              ...
              <small style={{ color: '#6b7280' }}>{I18N[lang].EnableHint}</small>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 6, flexWrap: 'wrap' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                >{I18N[lang].Upload}</button>
              </label>
              <button
                onClick={() => window.dispatchEvent(new Event('ddz-all-save'))}
                style={{ padding: '3px 10px', border: '1px solid #e5e7eb', borderRadius: 8, background: '#fff' }}
              >{I18N[lang].Save}</button>
            </div>

            <div style={{ marginTop: 10, borderTop: '1px dashed #eee', paddingTop: 10 }}>
              <div style={{ fontWeight: 700, marginBottom: 6 }}>局数</div>
              <label>局数
                <input type="number" min={1} value={rounds} onChange={e => setRounds(Number(e.target.value) || 1)} style={{ width: '100%' }} />
              </label>
            </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                {[0, 1, 2].map(i => (
                  <div key={i} style={{ border: '1px dashed #ccc', borderRadius: 8, padding: 10 }}>
                    ...
                    {/* API Key inputs for each AI provider */}
                    {seats[i] === 'ai:openai' && (
                      <label style={{ display: 'block', marginBottom: 6 }}>
                        OpenAI API Key
                    ...
                    {/* Similar for other providers... (gemini, grok, kimi, qwen, deepseek) */}
                    {seats[i] === 'http' && (
                      <>
                        <label style={{ display: 'block', marginBottom: 6 }}>
                          HTTP Base / URL
