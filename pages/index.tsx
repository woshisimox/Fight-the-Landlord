...
/**
 * index.tsx — 简洁稳定版（流式 NDJSON 解析 + 事件处理 + 基础UI）
 * - 只保留一处 type BotChoice 定义，避免 Duplicate identifier / Expression expected
 * - 兼容后端 /api/stream_ndjson 的 event/log/ka 等消息
 * - UI：对局设置、运行控制、牌面与日志
 */
...
export default function Home() {
  ...
  const [plays, setPlays] = useState<
    { seat: number; move: 'play' | 'pass'; cards?: string[]; comboType?: ComboType; reason?: string }[]
  >([]);
  ...
  const start = async () => {
    ...
    try {
      ...
      if (!resp.ok || !resp.body) {
        setLog((l) => [...l, `后端返回异常：${resp.status} ${resp.statusText}`]);
        setRunning(false);
        return;
      }
      ...
      for await (const line of ndjsonIterator(resp.body)) {
        ...
        try {
          ...
          setLiveLog((l) => [...l, `（非JSON行）${line.slice(0, 160)}`]);
          continue;
        }
        ...
        if ((m as any).type === 'event' && (m as any).kind === 'rob') {
          setLog((l) => [
            ...l,
            `${['甲', '乙', '丙'][(m as any).seat]} ${(m as any).rob ? '抢地主' : '不抢'}`,
          ]);
          continue;
        }
        ...
        // 兜底显示
        setLiveLog((l) => [...l, `未知消息：${line.slice(0, 160)}`]);
      }
    } catch (err: any) {
      if (err?.name === 'AbortError') {
        setLog((l) => [...l, '已停止']);
      } else {
        setLog((l) => [...l, `异常：${String(err)}`]);
      }
    } finally {
      setRunning(false);
    ...
    <div style={{ maxWidth: 1080, margin: '24px auto', padding: '0 16px' }}>
      ...
      <Section title="对局设置">
            ...
            <div key={i} style={{ border: '1px solid #eee', borderRadius: 8, padding: 10 }}>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const next = [...seatKeys];
                    next[i] = { ...(next[i] || {}), openai: e.target.value };
                    setSeatKeys(next);
                  }}
                  placeholder="sk-..."
                  style={{ width: '100%', marginTop: 6 }}
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const next = [...seatKeys];
                    next[i] = { ...(next[i] || {}), gemini: e.target.value };
                    setSeatKeys(next);
                  }}
                  placeholder="..."
                  style={{ width: '100%', marginTop: 6 }}
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const next = [...seatKeys];
                    next[i] = { ...(next[i] || {}), grok: e.target.value };
                    setSeatKeys(next);
                  }}
                  placeholder="..."
                  style={{ width: '100%', marginTop: 6 }}
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const next = [...seatKeys];
                    next[i] = { ...(next[i] || {}), kimi: e.target.value };
                    setSeatKeys(next);
                  }}
                  placeholder="..."
                  style={{ width: '100%', marginTop: 6 }}
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const next = [...seatKeys];
                    next[i] = { ...(next[i] || {}), qwen: e.target.value };
                    setSeatKeys(next);
                  }}
                  placeholder="..."
                  style={{ width: '100%', marginTop: 6 }}
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const next = [...seatKeys];
                    next[i] = { ...(next[i] || {}), httpBase: e.target.value };
                    setSeatKeys(next);
                  }}
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const next = [...seatKeys];
                    next[i] = { ...(next[i] || {}), httpToken: e.target.value };
                    setSeatKeys(next);
                  }}
                  placeholder="Bearer ..."
                  style={{ width: '100%', marginTop: 6 }}
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
          <button onClick={start} disabled={running} style={{ padding: '8px 14px' }}>
            ▶ 开始
          </button>
          <button onClick={stop} disabled={!running} style={{ padding: '8px 14px' }}>
            ■ 停止
          </button>
        </div>
      </Section>
