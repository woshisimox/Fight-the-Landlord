...
function LogLine({ text }:{ text:string }) {
  return <div style={{ fontFamily:'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize:12, whiteSpace:'pre-wrap' }}>{text}</div>;
}
    ...
    try {
        ...
        for (const raw of batch) {
          ...
          try {
            ...
            if (hasHands) {
              ...
              nextLandlord = lord;
              nextLog.push(`发牌完成，${lord!=null?['甲','乙','丙'][lord]:'?'}为地主`);
              continue;
            }

            if (m.type === 'event' && m.kind === 'rob') {
              nextLog.push(`${['甲','乙','丙'][m.seat]} ${m.rob ? '抢地主' : '不抢'}`);
              continue;
            }
      ...
      const pump = async (): Promise<void> => {
        while (true) {
          ...
          if (batch.length) {
            commitBatch(batch);
            // —— 核心修复：批量提交后，微让步一帧，防止 UI 被长批次“饿死” ——
      ...
      console.error(e);
      setLog(v => [...v, `[前端异常] ${e?.message || String(e)}`]);
    } finally {
      setRunning(false);
    }
  };
  ...
  return (
    <div>
      ...
      <Section title="手牌">
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:12 }}>
          {[0,1,2].map(i =>
            <div key={i} style={{ border:'1px dashed #eee', borderRadius:8, padding:'6px 8px' }}>
              <div style={{ marginBottom:6 }}>
                <SeatTitle i={i} />{landlord===i && <span style={{ marginLeft:6, fontSize:12, color:'#b45309' }}>（地主）</span>}
              </div>
              <Hand cards={hands[i]} />
            </div>
          )}
      ...
      <Section title="结果">
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:12 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end' }}>
            <button onClick={running ? stop : start} style={{ padding:'8px 14px', border:'1px solid #ddd', borderRadius:8, background: running ? '#fee2e2' : '#e0f2fe' }}>
              {running ? '停止' : '开始'}
            </button>
          </div>
        </div>
