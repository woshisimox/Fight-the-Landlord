...
export type GameState = {
  ...
  providerKeys: { openaiKey?: string; kimiKey?: string; grokKey?: string };
  playing: boolean;
  ...
  setProviderKeys(k: { openaiKey?: string; kimiKey?: string; grokKey?: string }): void;
  newRound(): void;
...
export const useGame = create<GameState>((set,get)=>({
  ...
  setProvider(seat,p){
    ...
    set({ providers: { ...get().providers, [key]: p } as any });
  },
  ...
  async stepPlay(auto=false){
    ...
    // 选择 provider 并调用（沿用全局 providerKeys）
    ...
    const newHistory: PlayRecord[] = [
      ...st.history,
      { seat, combo, text, reason: aiRes?.reason, provider }
    ];
