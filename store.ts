...
export const useGame = create<GameState>((set,get)=>({
  ...
  newRound(){
    ...
    let chosen = hand.filter(c=> chosenIds.includes(c.id));
    let combo = detectCombo(chosen) || { type:'pass', main:3 as any, cards:[] };

    if (isFreeLead) {
      if (combo.type === 'pass') {
        const min = [...hand].sort((a,b)=> a.rank-b.rank)[0];
        chosen = [min];
    ...
    let nextTurn = ((seat+1)%3) as Seat;
    let nextLast = st.lastCombo;
    let nextLeader = st.trickLeader;
    ...
    set({ hands:newHands, history:newHistory, turn:nextTurn, lastCombo:nextLast, trickLeader:nextLeader, passStreak:nextPassStreak, playing:!done });
  ...
  if (isFreeLead) {
    ...
    if (combo.type!=='pass' && !canBeat(combo, st.lastCombo!)) {
      combo = { type:'pass', main:3 as any, cards:[] };
    }
  }
  ...
  if (!done && auto){ await new Promise(r=>setTimeout(r,500)); await get().stepPlay(true); }
    ...
    let combo = detectCombo(chosen) || { type:'pass', main:3 as any, cards:[] };
    if (combo.type!=='pass' && st.lastCombo && !canBeat(combo, st.lastCombo)) combo = { type:'pass', main:3 as any, cards:[] };
