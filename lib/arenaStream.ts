// lib/arenaStream.ts
import {
  GreedyMax,
  GreedyMin,
  RandomLegal,
} from './engine';

import { OpenAIBot } from './bots/openai_bot';
import { GeminiBot } from './bots/gemini_bot';
import { GrokBot }   from './bots/grok_bot';
import { KimiBot }   from './bots/kimi_bot';
import { QwenBot }   from './bots/qwen_bot';
import { HttpBot }   from './bots/http_bot';

// 最小 IBot 类型，避免类型依赖问题
export type IBot = (ctx: any) => any | Promise<any>;

const seatLabel = (i: number) => '甲乙丙'.charAt(i) || `Seat${i + 1}`;

const asBot = (fn: IBot, _name?: string): IBot =>
  (ctx: Parameters<IBot>[0]) => fn(ctx);

export type BotSpec =
  | { kind: 'builtin'; name: 'greedy-max' | 'greedy-min' | 'random-legal' }
  | { kind: 'ai'; name: 'openai' | 'gemini' | 'grok' | 'kimi' | 'qwen'; model?: string; apiKey?: string }
  | { kind: 'http'; baseUrl: string; token?: string };

export function getBot(spec: BotSpec, seatIdx: number): IBot {
  const label = seatLabel(seatIdx);

  if (spec.kind === 'builtin') {
    if (spec.name === 'greedy-max') return asBot(GreedyMax, `${label}/GreedyMax`);
    if (spec.name === 'greedy-min') return asBot(GreedyMin, `${label}/GreedyMin`);
    return asBot(RandomLegal, `${label}/RandomLegal`);
  }

  if (spec.kind === 'ai') {
    const model = (spec.model || '').trim();

    if (spec.name === 'openai') {
      return asBot(OpenAIBot({ apiKey: spec.apiKey || '', model: model || 'gpt-4o-mini' }), `${label}/OpenAI`);
    }
    if (spec.name === 'gemini') {
      return asBot(GeminiBot({ apiKey: spec.apiKey || '', model: model || 'gemini-1.5-flash' }), `${label}/Gemini`);
    }
    if (spec.name === 'grok') {
      return asBot(GrokBot({ apiKey: spec.apiKey || '', model: model || 'grok-2-latest' }), `${label}/Grok`);
    }
    if (spec.name === 'kimi') {
      return asBot(KimiBot({ apiKey: spec.apiKey || '', model: model || 'moonshot-v1-8k' }), `${label}/Kimi`);
    }
    if (spec.name === 'qwen') {
      return asBot(QwenBot({ apiKey: spec.apiKey || '', model: model || 'qwen-plus' }), `${label}/Qwen`);
    }
  }

  if (spec.kind === 'http') {
    const base = (spec.baseUrl || '').replace(/\/$/, '');
    return asBot(HttpBot({ base, token: spec.token || '' }), `${label}/HTTP`);
  }

  return asBot(GreedyMax, `${label}/GreedyMax`);
}
