...
export function makeDeck(): Card[] {
  ...
  for (const r of ranks) for (const s of suits) {
    deck.push({
      id: `${r<=10?r:(r===11?'J':r===12?'Q':r===13?'K':r===14?'A':'2')}${s}`,
      rank: r as any, suit: s
    });
  }
...
function isRocket(cards: Card[]): Combo|null {
  return cards.length===2 && cards.every(c=>c.rank>=16)? { type:'rocket', main:17 as any, cards }: null;
}
function isBomb(cards: Card[]): Combo|null {
  if (cards.length===4){
    const m=counts(cards);
    return [...m.values()][0]===4? { type:'bomb', main:cards[0].rank as any, cards }: null;
  }
  return null;
}
...
function isPairs(cards: Card[]): Combo|null {
  ...
  const sorted=sortByRank(cards); const r=sorted.map(c=>c.rank);
  if (r.some(x=>x>=15)) return null;
  for (let i=0;i<r.length;i+=2){
    if (r[i]!==r[i+1]) return null;
    if (i>0 && r[i]!==r[i-2]+1) return null;
  }
  if (r.length<6) return null;
  return { type:'pairs', main:r[0] as any, cards };
}

export function detectCombo(cards: Card[]): Combo|null {
  if (cards.length===0) return { type:'pass', main:3 as any, cards:[] };
  return isRocket(cards) || isBomb(cards) || isSingle(cards) || isPair(cards) ||
         isTriple(cards) || isPairs(cards) || isStraight(cards);
}

export function canBeat(a: Combo, b: Combo|null): boolean {
  if (!b) return a.type!=='pass';
...
export function removeCards(from: Card[], take: Card[]): Card[] {
  const f=[...from];
  for (const t of take){
    const i=f.findIndex(x=>x.id===t.id);
    if (i>=0) f.splice(i,1);
  }
  return f;
}
