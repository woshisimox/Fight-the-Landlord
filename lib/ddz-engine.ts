import { Card, Combo, RANK_ORDER, sortByRank, code } from './ddz-types';
...
export function deal(deck: Card[]): [Card[],Card[],Card[],Card[]] {
  ...
  bottom.push(a[51],a[52],a[53]);
  return [sortByRank(p0), sortByRank(p1), sortByRank(p2), bottom];
}
...
function isStraight(cards: Card[]): Combo|null {
  const r = sortByRank(cards).map(c=>c.rank);
  if (r.length<5) return null; if (r.some(x=>x>=15)) return null;
  for (let i=1;i<r.length;i++) if (r[i]!==r[i-1]+1) return null;
  return { type:'straight', main:r[0] as any, cards };
}
function isPairs(cards: Card[]): Combo|null {
  ...
  for (let i=0;i<r.length;i+=2){ if (r[i]!==r[i+1]) return null; if (i>0 && r[i]!==r[i-2]+1) return null; }
  if (r.length<6) return null; return { type:'pairs', main:r[0] as any, cards };
}
function isAir(cards: Card[]): Combo|null {
  ...
  if (triples.length<2) return null; if (triples.some(x=>x>=15)) return null;
  for (let i=1;i<triples.length;i++) if (triples[i]!==triples[i-1]+1) return null;
...
export function canBeat(a: Combo, b: Combo|null): boolean {
  ...
  if (a.type===b.type && a.cards.length===b.cards.length) return a.main>b.main;
  return false;
}
...
export function codes(cards: Card[]): string[] { return cards.map(code); }
export function allLegalResponses(hand: Card[], last: Combo|null): Combo[] {
  const res: Combo[] = []; const n=hand.length;
  for (let i=0;i<n;i++) res.push(detectCombo([hand[i]])!).filter(Boolean);
  for (let i=0;i<n;i++) for (let j=i+1;j<n;j++) res.push(detectCombo([hand[i],hand[j]])!).filter(Boolean);
  for (let i=0;i<n;i++) for (let j=i+1;j<n;j++) for (let k=j+1;k<n;k++) res.push(detectCombo([hand[i],hand[j],hand[k]])!).filter(Boolean);
  for (let i=0;i<n;i++) for (let j=i+1;j<n;j++) for (let k=j+1;k<n;k++) for (let l=k+1;l<n;l++) res.push(detectCombo([hand[i],hand[j],hand[k],hand[l]])!).filter(Boolean);
  const sorted=sortByRank(hand);
  for (let len=5;len<=8;len++) for (let i=0;i<=sorted.length-len;i++){ const c=detectCombo(sorted.slice(i,i+len)); if (c&&c.type==='straight') res.push(c); }
  for (let len=6;len<=10;len+=2) for (let i=0;i<=sorted.length-len;i++){ const c=detectCombo(sorted.slice(i,i+len)); if (c&&c.type==='pairs') res.push(c); }
  for (let len=6;len<=12;len++) for (let i=0;i<=sorted.length-len;i++){ const c=detectCombo(sorted.slice(i,i+len)); if (c&&(c.type==='air'||c.type==='airWings')) res.push(c); }
  const unique: Combo[]=[]; const seen=new Set<string>();
  for (const c of res){ if (!c) continue; const k=c.type+'|'+c.cards.map(x=>x.id).join(','); if (!seen.has(k)){ seen.add(k); unique.push(c); } }
