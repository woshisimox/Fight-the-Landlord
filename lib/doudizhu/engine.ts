...
for (let s=0;s<3;s++) {
      ...
      const __external = (__choice.startsWith('ai:') || __choice === 'http' || __choice.startsWith('http'));
...
// thresholds for both built-ins && external choices (inline for scope)
      ...
      // __choice already declared above
...
if (__external) { try { yield { type:'event', kind:'bid-eval', seat: s, source:'external-ai', decision: (bid ? 'bid' : 'pass') }; } catch{} }
else { yield { type:'event', kind:'bid-eval', seat: s, score: sc, threshold: __th, decision: (bid ? 'bid' : 'pass') }; }
if (bid) {
        __bidders.push({ seat: s, score: sc, threshold: __th, margin: sc - __th });
        multiplier = Math.min(64, Math.max(1, (multiplier || 1) * 2));

        last = s;
yield { type:'event', kind:'bid', seat:s, bid, ...( __external ? { source:'external-ai' } : { score: sc } ), bidMult: bidMultiplier, mult: multiplier };
      }
      ...
      if (__bidders.length > 0) {
        ...
        for (let t = 0; t < 3; t++) {
          ...
          yield ( (()=>{ const __choiceT = String((bots as any)[t]?.choice || '').toLowerCase(); const __externalT = (__choiceT.startsWith('ai:') || __choiceT === 'http' || __choiceT.startsWith('http')); return (__externalT ? { type:'event', kind:'rob2', seat:t, source:'external-ai', bidMult: bidMultiplier, mult: multiplier } : { type:'event', kind:'rob2', seat: t, score: hit.score, threshold: hit.threshold, margin: Number((hit.margin).toFixed(4)), bidMult: bidMultiplier, mult: multiplier } ); })() );
...
{ const __choiceL = String((bots as any)[Lseat]?.choice || '').toLowerCase(); const __externalL = (__choiceL.startsWith('ai:') || __choiceL === 'http' || __choiceL.startsWith('http')); if (__externalL) { try { yield { type:'event', kind:'double-decision', role:'landlord', seat:Lseat, source:'external-ai' }; } catch{} } else try { yield { type:'event', kind:'double-decision', role:'landlord', seat:Lseat, double:!!Lflag, delta: lordDecision.delta, reason: lordDecision.reason }; } catch{}

 }// 乙（下家）：蒙特卡洛 + 反制能力
const yBase = __decideFarmerDoubleBase(hands[Yseat], bottom, __DOUBLE_CFG.mcSamples);
{ const __choiceF = String((bots as any)[Yseat]?.choice || '').toLowerCase(); const __externalF = (__choiceF.startsWith('ai:') || __choiceF === 'http' || __choiceF.startsWith('http')); if (__externalF) { { const __choiceF = String((bots as any)[Bseat]?.choice || '').toLowerCase(); const __externalF = (__choiceF.startsWith('ai:') || __choiceF === 'http' || __choiceF.startsWith('http')); if (__externalF) { try { yield { type:'event', kind:'double-decision', role:'farmer', seat:Bseat, source:'external-ai' }; } catch{} } else try { yield { type:'event', kind:'double-decision', role:'farmer', seat:Yseat, source:'external-ai' }; } catch{}  }} else try { yield { type:'event', kind:'double-decision', role:'farmer', seat:Yseat, double:!!yBase.F, dLhat:yBase.dLhat, counter:yBase.counter }; } catch{}
