...
if (bid) {
        ...
        last = s;
yield { type:'event', kind:'bid', seat:s, bid, score:(__external?undefined:sc), source:(__external?'external-ai':undefined), reason:(__external?(__aiBidReason[s]??null):undefined), bidMult: bidMultiplier, mult: multiplier };
      }
  ...
  if (/^(ai:|ai$|http|openai|gpt|external)/.test(__choiceL)) {
    try {
      const ctxL:any = {
        ...
        require:null, canPass:true, landlord:Lseat, leader:Lseat, trick:0,
        history: [], currentTrick: [],
        handsCount: [hands[0].length, hands[1].length, hands[2].length],
        counts: {}, teammates: [], opponents: [ (Lseat+1)%3, (Lseat+2)%3 ],
        doubling: { policy:'independent', order:'landlord-first', already:{}, canDouble:true, multiplier, baseMultiplier:1 },
      };
      const mvL = await Promise.resolve(bots[Lseat](ctxL));
      const rL:any = (mvL||{});
  ...
  if (/^(ai:|ai$|http|openai|gpt|external)/.test(__choiceY)) {
    try {
      const ctxY:any = {
        ...
        history: [], currentTrick: [],
        handsCount: [hands[0].length, hands[1].length, hands[2].length],
        counts: {}, teammates: [ (Lseat+2)%3 ], opponents: [ Lseat ],
        doubling: { policy:'independent', order:'landlord-first', already:{}, canDouble:true, multiplier, baseMultiplier:1 },
      };
  ...
  if (/^(ai:|ai$|http|openai|gpt|external)/.test(__choiceB)) {
    try {
      const ctxB:any = {
        ...
        history: [], currentTrick: [],
        handsCount: [hands[0].length, hands[1].length, hands[2].length],
        counts: {}, teammates: [ (Lseat+1)%3 ], opponents: [ Lseat ],
        doubling: { policy:'independent', order:'landlord-first', already:{}, canDouble:true, multiplier, baseMultiplier:1 },
      };
      const mvB = await Promise.resolve(bots[Bseat](ctxB));
      const rB:any = (mvB||{});
      const rrawB = rB.reason ?? rB.explanation ?? rB.rationale ?? rB.why ?? rB.comment ?? rB.msg;
      if (typeof rrawB === 'string' && rrawB.trim()) __aiDoubleReason[Bseat] = rrawB.slice(0, 800);
