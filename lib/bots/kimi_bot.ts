...
function buildPrompt(ctx: BotCtx): string {
  ...
  return ['你是斗地主出牌助手。','请根据我的手牌、是否必须跟牌、以及出牌规则，给出最优出牌。','必须严格输出 JSON，形如：','{ "move": "play|pass", "cards": ["A","A"], "reason": "简要理由" }','',`手牌：${hand}`,`需跟：${req}`,`可过：${canPass}`,`四带二规则：${policy}`].join('\n');
}
export const KimiBot = (opts: KimiOpts): BotFunc => {
  const apiKey = opts.apiKey; const model = opts.model || 'moonshot-v1-8k';
  return async (ctx: BotCtx): Promise<BotMove> => {
    try {
      ...
      const reason = (parsed.reason && String(parsed.reason).trim()) ? String(parsed.reason).trim() : 'Kimi 给出的建议';
