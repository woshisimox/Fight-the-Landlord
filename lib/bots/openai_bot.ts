...
type OpenAIOpts = { apiKey: string; model?: string };
...
export const OpenAIBot = (opts: OpenAIOpts): BotFunc => {
  const apiKey = opts.apiKey; const model = opts.model || 'gpt-4o-mini';
  return async (ctx: BotCtx): Promise<BotMove> => {
    try {
      if (!apiKey) throw new Error('Missing OpenAI API Key'); const prompt = buildPrompt(ctx);
      const r = await fetch('https://api.openai.com/v1/chat/completions', {
        method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${apiKey}` },
        body: JSON.stringify({ model, messages: [
          { role: 'system', content: '严格仅输出一个JSON对象，不要额外文本。键包含 move/cards/reason。' },
      ...
      const reason = (parsed.reason ?? '').toString().trim() || 'OpenAI 给出的建议';
