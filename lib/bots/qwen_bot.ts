...
export const QwenBot = (opts: QwenOpts): BotFunc => {
  const apiKey = opts.apiKey; const model = opts.model || 'qwen-plus';
  return async (ctx: BotCtx): Promise<BotMove> => {
    try {
      if (!apiKey) throw new Error('Missing Qwen API Key'); const prompt = buildPrompt(ctx);
      ...
      const reason = (parsed.reason ?? '').toString().trim() || 'Qwen 给出的建议';
